...
const AnalyticsChatbot = () => {
  ...
  // Load available queries and views on component mount
  useEffect(() => {
    ...
    const loadViews = async () => {
      try {
        ...
        console.log('📡 Views response:', viewsList);
        
        if (viewsList && Array.isArray(viewsList)) {
          setViews(viewsList);
          console.log('✅ Loaded views:', viewsList);
          // Set first view as default if available
          if (viewsList.length > 0 && !selectedView) {
            setSelectedView(viewsList[0].view_name);
          }
        } else {
          console.log('❌ No views found in response');
        }
      } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Send the natural language query to the insights API with selected view
      const result = await apiService.executeInsightsQuery(userQuery, selectedView || null);
      ...
      if (result && result.success) {
        const botMessage = {
          ...
          page: result.page || 1,
          originalQuery: userQuery, // Store original query for pagination refetch
          tableName: selectedView || null
        };
  ...
  const handlePageChange = async (messageId, newPage) => {
    // Find the message that needs pagination
    const message = messages.find(m => m.id === messageId);
    if (!message || !message.sqlQuery) {
      console.error('Cannot find message or SQL query for pagination');
      return;
    }

    // Set loading state on the specific message (not global typing indicator)
    setMessages(prev => prev.map(msg => 
      msg.id === messageId
        ? { ...msg, isLoadingPage: true }
        : msg
    ));

    try {
      console.log(`📄 Fetching page ${newPage} for message ${messageId} using stored SQL query`);
      
      // Execute the stored SQL query directly with new page (no Flask call)
      const result = await apiService.executeSqlQuery(message.sqlQuery, newPage);
      
      if (result && result.success) {
        // Update the same message with new data and page (no new message created)
        setMessages(prev => prev.map(msg => 
          msg.id === messageId
            ? {
                ...msg,
        ...
        console.error('Failed to fetch paginated results');
        // Remove loading state on error
        setMessages(prev => prev.map(msg => 
          msg.id === messageId
            ? { ...msg, isLoadingPage: false }
            : msg
        ));
      }
    } catch (error) {
    ...
    <div className="h-full bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-xl m-4 mt-0 flex flex-col shadow-sm" style={{ height: 'calc(100vh - 64px)' }}>
      ...
      <div className="bg-red-800 dark:bg-red-900 border-b border-red-600 dark:border-red-700 p-4 rounded-t-xl">
        ...
        <div className="flex items-center gap-3">
          <label className="text-red-100 text-sm font-medium">Select View:</label>
          <select
            value={selectedView}
            onChange={(e) => setSelectedView(e.target.value)}
            className="flex-1 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 border border-red-600 dark:border-red-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400"
          >
        ...
        <p className="text-red-100 text-sm mt-2">AI-powered analytics insights & custom analysis</p>
      </div>
              ...
              {message.queryResults && (
                <div className="mt-3">
                  ...
                  <QueryResultTable 
                    ...
                    title="Query Results"
                    page={message.page || 1}
                    totalCount={message.totalCount || message.queryResults.length}
                    onPageChange={(newPage) => handlePageChange(message.id, newPage)}
                    isLoading={message.isLoadingPage || false}
                  />
                </div>
              )}
