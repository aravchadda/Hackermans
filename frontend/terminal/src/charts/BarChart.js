...
const BarChart = ({ 
    ...
    aggregation = 'sum' // Aggregation type: 'sum', 'avg', 'count', 'max', 'min'
}) => {
    // Use theme context for reactive theme detection
    const { isDark: isDarkMode } = useTheme();

    // Helper function to aggregate values based on aggregation type
    const aggregateValues = (values, aggType) => {
        ...
        switch (aggType) {
            ...
            case 'avg':
                const sum = values.reduce((s, val) => s + (parseFloat(val) || 0), 0);
        ...
        if (hasBackendMultiValueFields) {
            ...
            data.forEach(item => {
                ...
                yValueFields.forEach((fieldName, index) => {
                    ...
                    if (!groupedData[xValue][fieldName]) {
                        groupedData[xValue][fieldName] = [];
                    }
                    groupedData[xValue][fieldName].push(yValue);
                });
            });
            
            // Apply aggregation to grouped values
            Object.keys(groupedData).forEach(xValue => {
                yValueFields.forEach(fieldName => {
                    if (Array.isArray(groupedData[xValue][fieldName])) {
                        groupedData[xValue][fieldName] = aggregateValues(groupedData[xValue][fieldName], aggregation);
                    }
                });
            });
            ...
            data.forEach(item => {
                ...
                yFields.forEach((yFieldName, index) => {
                    ...
                    if (!groupedData[xValue][yFieldName]) {
                        groupedData[xValue][yFieldName] = [];
                    }
                    groupedData[xValue][yFieldName].push(yValue);
                });
            });
            ...
            Object.keys(groupedData).forEach(xValue => {
                yFields.forEach(yFieldName => {
                    if (Array.isArray(groupedData[xValue][yFieldName])) {
                        groupedData[xValue][yFieldName] = aggregateValues(groupedData[xValue][yFieldName], aggregation);
                    }
                });
            });
            ...
            Object.keys(groupedData).forEach(category => {
                if (Array.isArray(groupedData[category])) {
                    groupedData[category] = aggregateValues(groupedData[category], aggregation);
                }
            });
