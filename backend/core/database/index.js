...
const initializeTables = async () => {
  ...
  // Add custom_name column if it doesn't exist (migration)
  try {
    const columnExists = await runQuery(`
      SELECT COUNT(*) as count
      FROM sys.columns
      WHERE object_id = OBJECT_ID('custom_views') AND name = 'custom_name'
    `);
    
    if (columnExists[0].count === 0) {
      await runQuery(`
        ALTER TABLE custom_views
        ADD custom_name NVARCHAR(255)
      `);
      console.log('✅ Added custom_name column to custom_views');
    }
  } catch (error) {
    console.log('Migration check for custom_name:', error.message);
  }

  // Add dateTime column if it doesn't exist (migration)
  try {
    const columnExists = await runQuery(`
      SELECT COUNT(*) as count
      FROM sys.columns
      WHERE object_id = OBJECT_ID('custom_views') AND name = 'dateTime'
    `);
    
    if (columnExists[0].count === 0) {
      await runQuery(`
        ALTER TABLE custom_views
        ADD dateTime NVARCHAR(255)
      `);
      console.log('✅ Added dateTime column to custom_views');
    }
  } catch (error) {
    console.log('Migration check for dateTime:', error.message);
  }
...
const restoreDatabaseBackup = async (backupFilePath) => {
  ...
  try {
    ...
    if (containerId) {
      ...
      const backupFileName = fileName.endsWith('.bak') ? fileName : `${fileName}.bak`;
      backupPathInContainer = `/var/opt/mssql/backup/${backupFileName}`;
      
      // Check if file is already in the mounted volume (uploads folder is mounted to /var/opt/mssql/backup)
      try {
        const checkResult = execSync(`docker exec ${dockerContainerId} test -f "${backupPathInContainer}"`, { stdio: 'pipe', encoding: 'utf-8' });
        console.log('Backup file found in mounted volume, using it directly');
      } catch (e) {
        // File not in mounted volume, need to copy it
