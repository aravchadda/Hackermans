...
MODEL_NAME = "gemma3:12b"  # Using available model
...
def call_ollama_insights(prompt, Column_descriptions, table_name='shipments', model=MODEL_NAME):
    """
    Call Ollama API specifically for insights query mapping
    """
    try:
        # Update the system prompt to use the actual table/view name
        system_prompt_template = INSIGHTS_SYSTEM_PROMPT.replace(
            'The table named `shipments` contains the following columns:',
            f'The table/view named `{table_name}` contains the following columns:'
        )
        system_prompt = f"{system_prompt_template.format(Column_descriptions=Column_descriptions)}"
        full_prompt = f"{system_prompt}\n\nUser: {prompt}\nOutput:"
        print("=" * 80)
...
def process_insights_query():
    ...
    try:
        ...
        user_query = data['query']
        table_name = data.get('tableName', 'shipments')  # Default to 'shipments' if not provided
        print(f"User insights query: {user_query}")
        print(f"Table/View name: {table_name}")
        logger.info(f"Received insights query: {user_query} for table/view: {table_name}")
        Column_descriptions = data.get('Column_descriptions', '')
        print(f"Column descriptions: {Column_descriptions}")
        logger.info(f"Column descriptions: {Column_descriptions}")
        # Call Ollama to map the query to a predefined SQL query
        llm_response = call_ollama_insights(user_query, Column_descriptions, table_name)
